---
abilitymods:
  - STR: -2
  - DEX: 8
  - CON: 5
  - INT: —
  - WIS: 0
  - CHA: -3 
ac: "EAC: 26 KAC: 28" 
alignment: N
cr: 12
date created: Monday, January 1st 2024, 3:13:12 pm
date modified: Tuesday, July 30th 2024, 10:08:24 pm
defabilities:
  - name: "Defensive Abilities"
    desc: "[[Swarm Defenses Ex]], [[Void Adaptation Ex or Su]]"
  - name: "[[Immunity (Ex or Su)|Immunities]]"
    desc: "cold, [[Swarm Immunities Ex]]"
EAC: 26
Ecology:
  - name: "Environment"
    desc: "any vacuum"
  - name: "Organization"
    desc: "swarm, pair, or brood (3–6)"
Environment: any vacuum
hp: 200
Init: 8
KAC: 28
Melee:
  - name: "Melee"
    desc: "swarm attack (6d6 C & P plus chilling toxin)"
modifier: 8
name: "Vermin (Space), Comet Wasp Swarm"
image: "[[space vermin, comet wasp swarm.png]]"

offabilities:
  - name: "Offensive Abilities"
    desc: "[[Distraction (Ex)]] (DC 19), implant"
otherabil: "[[Mindless Ex]]"
Perception: 22
reach: "0 ft."
saves:
  - Fort: 14
  - Ref: 16
  - Will: 11 
Senses: "[[blindsight]] (vibration) 30 ft., [[darkvision]] 60 ft."
size: Diminutive
Skills:
  - Acrobatics: 22
source: Alien Archive 2 
space: "10 ft."
specialabil:
  - name: "Implant (Ex)"
    desc: "A comet wasp swarm can choose not to use its swarm attack on (and deal no damage to) a helpless or paralyzed living creature in its space. Instead of doing so, the swarm implants eggs in that creature, afflicting it with comet wasp gestation."
Speed: 30 ft., fly 30 ft. (Su, perfect) 
statblock: true
subtype: (swarm)
tags: [Diminutive, Neutral, swarm, vermin]
Type: "N Diminutive vermin (swarm)"
type: vermin
XP: 19200 
---

```statblock
creature: Vermin (Space), Comet Wasp Swarm
```

[[Chilling Toxin]]
[[Comet Wasp Gestation]]

## Comet Wasp Swarm Cr 12"

Even the vacuum of space has bugs. Most of these void-dwelling vermin are harmless, but some are dangerous or predatory.
Asteroid lice and planetoid beetles feed on minerals in the rock of their airless habitats. Asteroid lice are communal, but planetoid beetles are territorial, tolerating only one mate. These creatures and their eggs can float in space to a new home. However, careless quarrying and shipping have accelerated their spread through the galaxy.
Xenobiologists theorize many space vermin evolved in special circumstances. Necropedes were once centipede-like scavengers on Eox
they now have a bite that can digest undead flesh. A warpmoth is like a glowing blue-white speck attracted to moving light, such as other warpmoths or passing starships. Gathering in swarms and following ships helps the moths feed and mate. Comet wasps create nests in frozen interstellar bodies. These icy, venomous vespids lay their eggs in living creatures.
